...
自主探索功能会实时获取机器人状态、空间状态和任务状态信息，通过自主探索环境和管理系统状态，在用户无感的情况下，高效的完成复杂的任务。
实现开机即用（用户直接和机器人交互，无需预先建好图），越用越聪明。
...
<img src="intro_images/intro_014.gif" width="800">
...
| :---: | :---: |
| <img src="intro_images/intro_013.gif" height="250"> | <img src="intro_images/intro_015.gif" height="250"> |
...
注2：采用8G版本RDK X5，可用内存大小6.9GB（% 相对 6.9GB 计），**地图面积 200㎡**。统计进程的 RSS 数据，另整机系统占用（`free -m` used，含 kernel/桌面/工具）约 1429MB（20.7%）。
...
2. 更换移动底盘方法参考[适配其他底盘](#8-适配其他底盘)章节。
3. 如果您没有移动底盘，仍然能够运行VSLAM（VIO和3D建图），具体方法参考[手持建图](#75-手持建图)章节。
>
...
如果您没有移动底盘，仍然能够运行VSLAM（VIO和3D建图）示例，具体方法参考[手持建图](#75-手持建图)章节。
...
| 基础配置项 | 参数 | 配置方法 |
| :---: | :---: | :---: |
| 相机和底盘外参 | robot_to_camera_x <br> robot_to_camera_y <br> robot_to_camera_z <br> robot_to_camera_roll <br> robot_to_camera_pitch | [相机和底盘外参标定](#61-相机和底盘外参标定粗标定)章节 |
| 底盘高度 | rtabmap_Grid_MaxObstacleHeight <br> max_obstacle_height | [底盘高度标定](#62-底盘高度标定)章节 |
| 验证标定参数 | —— | [相机和底盘外参标定](#61-相机和底盘外参标定粗标定)章节中【使用（验证）标定参数】小节 |
...
- 参考[RDK X5上安装双目相机](#31-rdk-x5上安装双目相机)和[软件配置](#4-软件配置)章节完成相机的安装和配置。
...
参考[外参标定](#6-外参标定)章节完成相机和底盘之间的外参标定。
...
- **定位更稳**：VIO在反光、玻璃、长走廊、大白墙等弱纹理场景下定位稳定性显著提升。对比0.0.5版本，在234个数据集的测试中，跑飞（轨迹首尾误差大于20m）率为 8.9%，降低约 43.6%，大幅位置跳变（单帧位置变化大于2m）概率为 1.3%，减少约 77%。
